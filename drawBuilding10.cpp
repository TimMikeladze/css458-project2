...
#define PI 3.14159
 //additional elements
float brickMat[] = { 0.58823529411, 0.08623730939, 0.0431372549, 1};
	 float white[] = { 0.95, 0.95, 0.95, 1};
	 float blueWindows[] = {1,1,1,0.25};
 void output() {
	 float shift = 25;
	 glPushMatrix();
  glRotatef(360, 0, 1, 0);
...
void firstFloorLedge(double x, double y, double z) {
	float yValue = 0.15;
	float zValue = -0.5;
	//pillars
	//farthest left
...
void windowTop(bool recessed, bool isThirdFloor) {
	if(recessed) {
		if(isThirdFloor) {
			//nonrecessed
			glPushMatrix();
...
void windowLeftRight(float xValue) {
	   glPushMatrix();
...
void windowMiddle(bool recessed) {
		if(recessed) {
		glPushMatrix();
...
void window(bool recessed, bool isThirdFloor, bool isFirstFloor) {
	//top
 ...
 void ArchFaceVault(float xValue, float resize) {
	 glPushMatrix();
	int i, numSlices;
